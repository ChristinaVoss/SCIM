...
class CreateClub(FlaskForm):
    ...
    end_time = TimeField('End time*', format='%H:%M', validators=[DataRequired(message='Please enter a start time between 6AM and 11PM.')])
    start_time = TimeField('Start time*', format='%H:%M', validators=[DataRequired(message='Please enter end time, it must be later than start time.')])
...
class ChooseDay(FlaskForm):
    submit_day = SubmitField('')
